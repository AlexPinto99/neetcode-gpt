...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.attention_mechanism = self.MultiHeadedSelfAttention(model_dim, num_heads)

        # 2. Feedforward Network
        self.feed_forward = self.VanillaNeuralNetwork(model_dim)

        # 3. LayerNorm instances
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        embedded = embedded + self.attention_mechanism(self.layer_norm_1(embedded))
